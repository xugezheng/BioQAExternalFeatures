...
def main_treat(filename):
    ...
    error_num = 0
    ner_tag_dict = {}
    ner_tags = ["PERSON", "NORP", "ORG", "LOC", "DATE", "TIME", "PERCENT", "GPE", "PRODUCT", "QUANTITY", "ORDINAL", "CARDINAL"]
                ...
                for ner in question_span.ents:
                    if ner.label_ in ner_tags:
                        ...
                        if ner.text in q_word:
                            ...
                            tokenized_ner = tokenizer.tokenize(q[ner.start_char:ner.end_char])
                            # print(q_word)
                            # print(tokenized_ner)
...
#             filename = "pos_BioASQ-test-" + type + '-' + year + '-' + str(batch) + ".json"
#             main_treat(filename)
#             print(filename + " has been treated.")

#main_treat("all_json/ner_pos_train-v1.1.json")
# vocab_file = "vocab.txt"
# tok = tokenization.FullTokenizer(vocab_file=vocab_file, do_lower_case=False)
# a = tok.tokenize('Which enzyme is targeted by Evolocumab? Antibody therapeutics in Phase 3 studies are described,…')
# print(a)

# main_treat("pos_train-v1.1.json")

# main_treat("pos_BioASQ-train-factoid-7b-full-annotated.json")

main_treat("pos_BioASQ-train-factoid-8b-full-annotated.json")

# for batch in range(1,6):
#     #filename = "BioASQ-" + year + "/test/Full-Abstract/BioASQ-test-" + type + '-' + year + '-' + str(batch) + ".json"
#     filename = "pos_BioASQ-test-factoid-8b-" + str(batch) + ".json"
#     main_treat(filename)
#     print(filename + " has been treated.")
#     filename = "pos_BioASQ-test-factoid-8b-" + str(batch) + "-snippet.json"
#     main_treat(filename)
#     print(filename + " has been treated.")
